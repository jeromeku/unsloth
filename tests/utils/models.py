...
class Model:
    ...
    def __post_init__(self):
        self.name = f"{self.base_name}-{self.version}-{self.size}B"
        if self.quant_type:
            if self.quant_type == "bnb":
                self.name = "-".join([self.name, BNB_QUANTIZED_TAG])
            elif self.quant_type == "unsloth":
                self.name = "-".join([self.name, UNSLOTH_DYNAMIC_QUANT_TAG])

        if self.instruct_tag:
            self.name = "-".join([self.name, self.instruct_tag])
...
def register_model(
    ...
    print(f"Registering model {key}, current registry: {list(MODEL_REGISTRY.keys())}")
